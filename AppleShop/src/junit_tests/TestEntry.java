...
import model.*; // * means it imports all classes from model package
...
public class TestEntry {
	...
	public void test_entry_1() {
		...
		//	assertTrue(e.getSerialNumber() =="34"); // not necessarily working always, why?
		assertEquals("F9DN4NKQ1GC", e.getSerialNumber());
		assertTrue (e.getProduct() == p);
		assertSame(e.getProduct(), p); // are e.getProduct() and p referring to the same Product objects (or are they the same Product address)
	...
	public void test_entry_2() {
		...
		p2.setFinish("Gold");
		p2.setStorage(64); // 64 GB
		p2.setHasCellularConnectivity(false); // unnecessary cause p2 is already initialized as false
